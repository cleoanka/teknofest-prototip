...
> Son güncelleme: 2026-07-13 · Durum: v2.3 — Backend v1.5 · **358 test** · Plaka YOLO11n + crop/takip + metrik hız · CRITICAL model tip-ayrımlı v4 (yolov8m@768, test mAP50 .788)
...
│                                                                     │
│  ┌──────────────┐  ┌────────────┐  ┌────────────┐  ┌────────────┐   │
│  │ detector.py  │  │tracking.py │  │calibration │  │lp_detector │   │
│  │ YOLOv8n/s    │  │ IOU takip  │  │ Metrik hız │  │YOLO11n     │   │
│  │ (gerçek/mock)│  │ track_id   │  │ oto-kalib  │  │oto-indirme │   │ 
│  └──────┬───────┘  └─────┬──────┘  └─────┬──────┘  └─────┬──────┘   │ 
│         └────────────────┴─────────┬─────┴───────────────┘          │
│                                    │                                │
│  ┌──────────────┐  ┌───────────────▼──────┐  ┌────────────────┐     │
│  │plate_crop.py │  │ plate_tracker.py     │  │ plate_ocr.py   │     │
│  │Siyah çerçeve │  │ Araç-id'ye bağlı     │  │ EasyOCR çok-   │     │
│  │takip + deskew│  │ plaka kararlılığı    │  │ varyant + kons.│     │
│  │likeness geçid│  │ (ttl prune)          │  │                │     │
│  └──────┬───────┘  └────────────┬─────────┘  └────────┬───────┘     │
│         └───────────────────────┴──────────────────────┘            │
│                                    │                                │
│  ┌──────────────┐  ┌───────────────▼──────┐  ┌────────────────┐     │
...
[Letterbox 416/640 + ROI crop]
        │
        ▼
[YOLOv8n/s → NMS(conf=0.25, iou=0.45) → IOU takip (track_id)]
        │
        ├─ Blok A │ Kabin ROI → YOLO sınıf tespiti (phone/cigarette/seatbelt/headphone)
        │
        ├─ Blok B │ Yüz ROI → MediaPipe Face Mesh (468 landmark)
        │          │           EAR 8-frame pencere → PERCLOS → yorgunluk
        │
        ├─ Blok C │ Metrik hız oto-kalibrasyonu
        │          │  → ppm(y) ölçek-alanı: plaka gözlemi (yüksek ağırlık) +
        │          │    araç-genişliği gözlemi (düşük ağırlık yedek)
        │          │  → vanishing point / homografi (opsiyonel)
        │          │  → metrik km/h (kalibrasyonsuz bbox-alan yedeği)
        │
        └─ Blok D │ Araç crop → YOLO11n plaka dedektörü (morsetechlab, ~5 MB)
                   │  → looks_like_plate() geçidi (aspect/kontrast/dikey-kenar)
                   │  → refine_to_frame() siyah çerçeveye sıkılaştır + deskew
                   │  → plate_sharpness() Laplacian → EasyOCR (4 varyant)
                   │  → PlateTracker: araç track_id'ye bağlı kararlı okuma
                   │  → plate_pixel_width → kalibrasyona geri besleme
                   ▼
[QoD Tetik Motoru A–E] + [Risk: Tel(40)+Sig(20)+Yorg(30)+Hız(15)+Zigzag(10)]
                   │
                   ▼ JSON < 3 KB
           [WebSocket → Mobil UI / Web Konsol]
```

**Plaka tespiti (v2.3):** Mükemmel dikdörtgen şart değil — `refine_to_frame()` ROI içinde
plakanın koyu çerçevesini `minAreaRect` ile bulup ona sıkılaştırır. Plaka karede yoksa
`PlateTracker` son bilinen okumayı korur (araç-id süreklilik). Saçma bölgeler (trafik
levhası, far, panel) `looks_like_plate()` geçidinde elenir. SSL güvenlik açığı (global
doğrulama kapatma) da bu versiyonda kapatıldı — certifi CA yalnız EasyOCR bloğuna sarılı.
...
│   ├── lp_detector.py               Plaka tespiti (YOLO11n oto-indirme + CV yedek)
│   ├── plate_crop.py                Siyah-çerçeve takip crop + deskew + likeness geçidi
│   ├── plate_tracker.py             Araç-id'ye bağlı plaka kararlılığı (ttl prune)
│   ├── plate_ocr.py                 Çok-varyant OCR + konsensüs (SSL düzeltildi)
...
| **Plaka tespiti v2.3** — YOLO11n oto-indirme + siyah-çerçeve crop + araç-id takibi | ✅ Tamamlandı (7 Haz) |
| Metrik hız oto-kalibrasyonu (ppm/homografi/VP) | ✅ Tamamlandı |
| QoD eval — gerçek video ~%43 bant tasarrufu (%40 şartname kriteri ✓) | ✅ Tamamlandı |
